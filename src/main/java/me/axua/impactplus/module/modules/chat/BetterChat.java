  ...
  private Listener<ClientChatReceivedEvent> chatReceivedEventListener = new Listener<>(event -> {
    ...
    if (this.friendHighlight.getValBoolean() && !event.getMessage().getUnformattedText().contains("<" + mc.player.getName() + ">")) {
      Friends.getFriends().forEach(friend -> {
        if(event.getMessage().getUnformattedText().contains(friend.getName())) {
          String newName = ChatFormatting.AQUA + friend.getName() + " " + ChatFormatting.RESET;
          friend.getName().replaceAll(friend.getName(), newName);
        }
      });
    }
