...
public class ClickGUI extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    ImpactPlus.fontRenderer.getClass();
    ImpactPlus.fontRenderer.drawStringWithShadow("by Axua", -ImpactPlus.fontRenderer.getStringWidth("by Axua"), -9, this.text.getRGB());
    GL11.glPopMatrix();
    this.mb = null;
    ...
    if (this.mb != null) {
      ...
      ImpactPlus.fontRenderer.drawCenteredStringWithShadow("Listening...", 0.0F, -10.0F, -1);
      GL11.glScalef(0.5F, 0.5F, 0.0F);
      ImpactPlus.fontRenderer.drawCenteredStringWithShadow("Press 'ESCAPE' to unbind " + this.mb.mod.getName() + ((this.mb.mod.getBind() > -1) ? (" (" + Keyboard.getKeyName(this.mb.mod.getBind()) + ")") : ""), 0.0F, 0.0F, -1);
      GL11.glPopMatrix();
    }
    for (Panel p : panels) {
      for (ModuleButton b : p.Elements) {
        if (b.isHovered(mouseX, mouseY) && (ImpactPlus.getInstance()).settingsManager.getSettingByID("ClickGuiTooltips").getValBoolean()) {
          ...
          if ((ImpactPlus.getInstance()).settingsManager.getSettingByID("ClickGuiStyle").getValString().equals("Old")) {
            Gui.drawRect(xx - 2, yy - 2, xx + (int)ImpactPlus.fontRenderer.getStringWidth(desc) + 2, yy + FontUtil.getFontHeight() + 2, outlineColor);
            ImpactPlus.fontRenderer.drawStringWithShadow(desc, (xx - 1) + 10, (yy - 2), -1);
            continue;
          }
          if ((ImpactPlus.getInstance()).settingsManager.getSettingByID("ClickGuiStyle").getValString().equals("WeepCraft")) {
          ...
          Gui.drawRect(xx - 2, yy - 2, xx + (int)ImpactPlus.fontRenderer.getStringWidth(desc) + 2, yy + FontUtil.getFontHeight() + 2, -1777134829);
          ImpactPlus.fontRenderer.drawCenteredStringWithShadow(desc, (xx - 1), (yy - 2), -1);
        }
      }
    }
