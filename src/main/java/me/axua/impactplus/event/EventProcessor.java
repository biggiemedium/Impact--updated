        ...
        for (SPacketPlayerListItem.AddPlayerData playerData : packet.getEntries()) {
          if (playerData.getProfile().getId() != this.mc.session.getProfile().getId())
            (new Thread(() -> {
              ...
              if (name != null && mc.player != null && mc.player.ticksExisted >= 1000) {
                ImpactPlus.EVENT_BUS.post(new PlayerJoinEvent(name));
              }
            })).start();
        }
        ...
        for (SPacketPlayerListItem.AddPlayerData playerData : packet.getEntries()) {
          if (playerData.getProfile().getId() != this.mc.session.getProfile().getId())
            (new Thread(() -> {
              String name = resolveName(playerData.getProfile().getId().toString());
              if (name != null && mc.player != null && mc.player.ticksExisted >= 1000) {
                ImpactPlus.EVENT_BUS.post(new PlayerLeaveEvent(name));
              }
            })).start();
        }
    }
