...
public class FpsComponent extends Panel {
  ...
  public void drawHud() {
    ...
    float hi = ImpactPlus.fontRenderer.getStringWidth(fps);
    float hi2 = mc.fontRenderer.getStringWidth(fps);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(fps, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(fps, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.fontRenderer.drawStringWithShadow(fps, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (this.mod.numbercolour.getValString().equalsIgnoreCase("Gold"))
      numbercf = ChatFormatting.GOLD; 
    String fps = "FPS " + numbercf + Minecraft.getDebugFPS();
    double w = (ImpactPlus.fontRenderer.getStringWidth(fps) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.fontRenderer.drawStringWithShadow(fps, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(fps, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(fps, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
