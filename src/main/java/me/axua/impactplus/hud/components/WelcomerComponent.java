...
public class WelcomerComponent extends Panel {
  ...
  public void drawHud() {
    doStuff();
    float hi = ImpactPlus.fontRenderer.getStringWidth(this.msg);
    float hi2 = mc.fontRenderer.getStringWidth(this.msg);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(this.msg, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(this.msg, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.fontRenderer.drawStringWithShadow(this.msg, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    doStuff();
    double w = (ImpactPlus.fontRenderer.getStringWidth(this.msg) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.fontRenderer.drawStringWithShadow(this.msg, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(this.msg, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(this.msg, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
