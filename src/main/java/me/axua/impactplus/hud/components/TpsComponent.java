...
public class TpsComponent extends Panel {
  ...
  public void drawHud() {
    ...
    float hi = ImpactPlus.fontRenderer.getStringWidth(tps);
    float hi2 = mc.fontRenderer.getStringWidth(tps);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(tps, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(tps, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.fontRenderer.drawStringWithShadow(tps, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (this.mod.numbercolour.getValString().equalsIgnoreCase("Gold"))
      numbercf = ChatFormatting.GOLD; 
    String tps = "TPS " + numbercf + this.decimalFormat.format(TpsUtils.getTickRate());
    double w = (ImpactPlus.fontRenderer.getStringWidth(tps) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.fontRenderer.drawStringWithShadow(tps, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(tps, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(tps, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
