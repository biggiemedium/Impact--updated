...
public class TimeComponent extends Panel {
  ...
  public void drawHud() {
    ...
    float hi = ImpactPlus.fontRenderer.getStringWidth(date);
    float hi2 = mc.fontRenderer.getStringWidth(date);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(date, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(date, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.fontRenderer.drawStringWithShadow(date, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    Minecraft mc = Minecraft.getMinecraft();
    String date = (new SimpleDateFormat("h:mm a")).format(new Date());
    double w = (ImpactPlus.fontRenderer.getStringWidth(date) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.fontRenderer.drawStringWithShadow(date, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(date, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.fontRenderer.drawStringWithShadow(date, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
